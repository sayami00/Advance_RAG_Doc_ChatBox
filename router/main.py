...
from workflow.document_function import DocHandler,FastAPIFileAdapter
...
async def upload_process(
    ...
    try:
        for file in files:
            ...
            saved_path = await dh.temp_save_pdf(FastAPIFileAdapter(file), departmenttemppath)
            # ⚠️ Optional: only read if needed
